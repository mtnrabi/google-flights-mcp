...
def build_server(settings: Settings | None = None) -> FastMCP:
    ...
    async def openai_challenge(_request: Request) -> Response:
        ...
        nothing else -- no JSON envelope, no trailing newline, no list.
        
        This token is public verification data, not a secret, so it's safe to
        hardcode. It was generated by OpenAI for domain ownership verification.
        """
        token = os.environ.get("OPENAI_APPS_CHALLENGE_TOKEN", "").strip()
        if not token:
            token = "BAdPhuILiv3Dra7_fwWBueieYTR_c8U_ZLeCWKa6Yxs"
